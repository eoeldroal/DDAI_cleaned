...
"""
[DEPRECATED] rm_phase2_deprecated.py

이 파일은 더 이상 사용되지 않습니다.

기존 구현: Qwen2.5-72B-Instruct 기반 LLM Judge
- FastAPI 서버(model_eval_api.py)를 통한 간접 호출
- 최종 답변만 평가 (추론 과정 미포함)
- 점수 공식: 0.2 * model_eval_score + 0.8 * ndcg_value

새 구현은 rm_phase2.py를 참조하세요.
- Gemini 3 Flash 기반 VLM as Judge
- 추론 과정 전체 평가 (visual_claims, think_answer_consistency 등)
- 최적화된 단일 루프 구조
- 점수 공식: 0.8 * vlm_score + 0.2 * ndcg_value
"""
